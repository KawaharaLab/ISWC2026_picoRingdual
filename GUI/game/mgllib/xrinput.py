...
class XRInput(ElementSingleton):
    ...
    def init(self, context):
        if context is None:
            ...
            self.raw_head_pos = (0, 1.8, 0)
            self.raw_head_orientation = glm.quat()
            self.head_transform = np.identity(4)
            self.head_movement = (0, 0, 0)
            self.hands = [Controller(0), Controller(1)]
            self.vibrate_action = 0
            self.controller_paths = [0, 0]
    ...
    def update(self, frame_state):
        if self.context is None:
            # Mock update from Keyboard (Moved to Application level)
            return
