...
EXTI_HandleTypeDef CUSTOM_NFC07A1_GPO_EXTI={.Line=ST25DVXXKC_INT_PIN_GPO_EXTI_LINE};
...
int32_t CUSTOM_GPO_Init( void )
{
  ...
  (void)HAL_EXTI_GetHandle(&CUSTOM_NFC07A1_GPO_EXTI, ST25DVXXKC_INT_PIN_GPO_EXTI_LINE);
  (void)HAL_EXTI_RegisterCallback(&CUSTOM_NFC07A1_GPO_EXTI,  HAL_EXTI_COMMON_CB_ID, BSP_GPO_Callback);
...
void CUSTOM_NFC07A1_GPO_IRQHandler(void)
{
  HAL_EXTI_IRQHandler(&CUSTOM_NFC07A1_GPO_EXTI);
}
