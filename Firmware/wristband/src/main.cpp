 ...
 * @brief   Arduino test application for the STMicrolectronics X-NUCLEO-NFC08A1
...
#define ST25DV_MB_CTRL_DYN_REG      0x0D  /* Register address of dynamic configuration for mailbox control */
#define ST25DV_MB_CTRL_MB_EN 0x00  /* Enable FTM */
#define ST25DV_MB_CTRL_HOST_PUT_MSG   0x01  /* Host (I2C) posts a message */
#define ST25DV_MB_CTRL_RF_PUT_MSG 0x02  /* RF posts a message */
#define ST25DV_MB_CTRL_HOST_CURRENT_MSG 0x06 /* Current message comes from Host (I2C) */
#define ST25DV_MB_CTRL_RF_CURRENT_MSG 0x06 /* Current message comes from RF */
...
void ParseAndPrintImuData(uint8_t* buffer) {
  ...
  // 1. Extract binary data (Total 9 bytes)
  
  // Quaternion (int16_t x 4 = 8 bytes)
   ...
   * Format: "IMU:w,x,y,z,gpio"
   * Output quaternion with 4 decimal places of precision
   */
  Serial.print("IMU:");
...
#ifndef I2C_ENABLED
  RfalRfST25R3916Class rfst25r3916(&SPI, CS_PIN, IRQ_PIN);
#else
  #define dev_i2c Wire
...
int Test_CheckMailboxStatus(const uint8_t *uid);
int Test_ReadMessage(uint8_t *msg);
...
static void checkUserButton(void)
{
  ...
  //     timerLed = rfst25r3916.timerCalculateTimer(
